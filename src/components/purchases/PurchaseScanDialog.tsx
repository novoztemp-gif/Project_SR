...
import { extractPurchaseFromImage, type ParsedPurchase } from '@/lib/purchaseScan'

interface PurchaseScanDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  /** Called with both the parsed vendor/items AND the raw scanned image, so
   * the caller can auto-fill the form and keep the image as a reference photo. */
  onExtract: (parsed: ParsedPurchase, imageDataUrl: string) => void
  initialImageDataUrl?: string
  extractingLabel?: string
}

export function PurchaseScanDialog({
  open,
  onOpenChange,
  onExtract,
  initialImageDataUrl,
  extractingLabel = 'Extracting...',
}: PurchaseScanDialogProps) {
  ...
  React.useEffect(() => {
    ...
    async function startCamera() {
      ...
      if (!navigator.mediaDevices?.getUserMedia || !window.isSecureContext) {
        setCameraMessage('Camera preview needs localhost or HTTPS. You can upload an invoice image instead.')
        setMode('upload')
        return
      }
      ...
      try {
        ...
        setCameraMessage('Camera access was not available. Upload an invoice image to continue.')
        setMode('upload')
      }
    }
  ...
  async function handleExtract() {
    ...
    try {
      ...
      const parsed = await extractPurchaseFromImage(imageDataUrl)
      onExtract(parsed, imageDataUrl)
      handleOpenChange(false)
      toast.success('Invoice scanned — review and save')
    } catch (err) {
      const message = err instanceof ApiClientError ? err.message : 'Could not extract invoice details'
      toast.error(message)
    } finally {
      setIsExtracting(false)
    }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            AI reads the vendor invoice (image or PDF) and fills the form — needs the backend key configured.
          </DialogDescription>
        </DialogHeader>
