  ...
  return (
    <div className="printable-bill bg-white p-6 font-mono text-sm text-gray-800">
      {/* print-running-header is fixed-positioned in print only (see
          ...
          be. Everything below it is wrapped in print-content-block, whose
          top padding is *cloned* onto every page it fragments across (see
          index.css) — a plain one-off spacer only reserved that space on
          page 1, so anything that overflowed onto page 2 rendered right
          underneath the fixed header and was effectively invisible. */}
      ...
      <div className="print-content-block print-content-block--sales-bill">
        {/* ── Items table ── */}
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-300 bg-gray-100 text-gray-700">
              ...
              <th className="text-left py-1.5 font-medium pr-2 text-gray-700 whitespace-nowrap">Size</th>
          ...
          <tbody>
            {bill.items.map((item, i) => (
              <tr key={i} className="border-b border-gray-200 text-gray-800">
                <td className="py-1.5 pr-2 tabular-nums text-gray-600 whitespace-nowrap">{item.serialNumber || i + 1}</td>
                <td className="py-1.5 pr-2">{item.productName}</td>
                <td className="py-1.5 pr-2 text-gray-600 whitespace-nowrap">{item.glassSize ?? ''}</td>
                <td className="py-1.5 pr-2 text-right tabular-nums whitespace-nowrap">{item.quantity}</td>
                <td className="py-1.5 pr-2 text-gray-600 whitespace-nowrap">{item.model ?? ''}</td>
                <td className="py-1.5 pr-2 text-right tabular-nums whitespace-nowrap">{NUM.format(item.unitPrice)}</td>
                <td className="py-1.5 text-right tabular-nums whitespace-nowrap">{NUM.format(item.subtotal)}</td>
              </tr>
            ))}
            <tr>
              ...
              <td className="py-1.5 text-right tabular-nums font-semibold border-t border-gray-300 text-gray-900 whitespace-nowrap">
        ...
        {/* Totals + amount-in-words + footer move to the next page together
            as one block if they don't fit under the last item row — never
            split with e.g. Balance Amount stranded alone (see
