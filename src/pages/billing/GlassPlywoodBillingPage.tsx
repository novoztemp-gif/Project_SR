...
export function GlassPlywoodBillingPage() {
  ...
  const total = (watchedItems ?? []).reduce((sum, item) => {
    ...
    // Glass-only amounts — always 0 for a non-glass item (BillLineItem
    // hides these inputs and resets them whenever the selected product
    // isn't glass), so it's safe to add them in unconditionally here.
